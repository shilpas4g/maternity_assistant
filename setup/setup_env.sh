...
if [[ ! "$ENABLED_SERVICES" == *"mapstools.googleapis.com"* ]]; then
    gcloud --quiet beta services mcp enable mapstools.googleapis.com --project=$
fi
...
API_KEY_NAME="maternity-demo-key-$(date +%s)"
...
ENV_FILE="$SCRIPT_DIR/../adk_agent/maternity/.env"
